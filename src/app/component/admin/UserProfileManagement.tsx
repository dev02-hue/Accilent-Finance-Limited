...
import { FaEdit, FaSave, FaTimes, FaUser, FaEnvelope, FaPhone, FaDollarSign, FaGift } from "react-icons/fa";
    ...
    try {
      ...
      if (error) {
        ...
        setProfiles(data.map(profile => ({ 
          ...profile, 
          totalBonusAndInterest: profile.totalBonusAndInterest || 0,
          isEditing: false 
        })));
      }
    } catch (err) {
  ...
  const handleUpdate = async (profile: EditableProfile) => {
    try {
      const { success, error } = await updateUserProfile({
        ...
        totalBonusAndInterest: profile.totalBonusAndInterest
      });
        ...
        <table className="min-w-full bg-white rounded-lg overflow-hidden">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Bonus & Interest</th>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  {profile.isEditing ? (
                    <div className="flex items-center">
                      ...
                      <input
                        type="number"
                        value={profile.totalBonusAndInterest}
                        onChange={(e) => handleChange(profile.id, 'totalBonusAndInterest', parseFloat(e.target.value))}
                        className="w-full px-2 py-1 border rounded"
                        step="0.01"
                      />
                    </div>
                    ...
                    <div className="flex items-center">
                      <FaGift className="text-gray-500 mr-2" />
                      ${profile.totalBonusAndInterest.toFixed(2)}
                    </div>
                  )}
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
