    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          balance,
          total_bonus_and_interest: totalBonusAndInterest, // Added new field
  ...
  export async function getAllProfiles(): Promise<{ data?: Profile[]; error?: string }> {
    try {
      ...
      const { data: profiles, error } = await supabase
        .from('accilent_profile')
        .select('id, name, username, email, phone_number, balance, total_bonus_and_interest');
      ...
      const formatted = profiles.map(profile => ({
        ...
        totalBonusAndInterest: profile.total_bonus_and_interest || 0, // Added new field with default value
      }));
